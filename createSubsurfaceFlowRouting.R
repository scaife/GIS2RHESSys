...
# bounded by GIS mask
	...
	hashenvIII <- new.env(hash=T)
	list2env(setNames(as.list(maskRC),maskRC),envir= hashenvIII) #<<---- native R hash
	...
	patchNeighbourRC = sapply(patchNeighbourRC_LEN, function(ii){
		sapply(names(patchNeighbourRC_edge[[ii]]), function(x){hashenvIII[[ x ]]})
	})
	...
	patchPreferNeighbourRC = tapply(fullLength, INDEX=outputOrder, FUN=function(ii){
		...
		return <- sapply(names(tapply(hold3[hold3>0], hold3[hold3>0],length)),function(x){hashenvIII[[ x ]]})
	})
