...
#arg = c('dem','uaa','colmap','rowmap','drain','strHeadwaterbuff')
#arg = c('dem_bankcutB','uaa','colmap','rowmap','drain','strHeadwaterbuff')
#arg = c('dem_bankcutC','uaa','colmap','rowmap','drain','strHeadwaterbuff')
arg = c('dem_bankcutD','uaa','colmap','rowmap','drain','strHeadwaterbuff')

rast = readRAST(arg,NODATA=0)
mask = !is.na(rast@data[[1]])
dem = rast@data[[1]][mask]
uaa = rast@data[[2]][mask]
cols = rast@data[[3]][mask]
rows = rast@data[[4]][mask]
drain = rast@data[[5]][mask]
strbuff = rast@data[[6]][mask]
...
list2env(setNames(as.list(seq_along(dem)),maskRC),envir=hashenv) #<<---- native R hash
	...
	##############################################################################################
	### assume that strHeadwaterbuff distance class is by the grid resolution (e.g., by 3m or 10m)
	### grass buffet, always starts from class 1, which is distance 0 m.
	## ------ distance chances from channel
	# xx = seq(0,60,1); plot(xx,exp(-0.1*xx),type='l')
	# resolution = sqrt(gmeta()$nsres * gmeta()$ewres)
	# cond = !is.na(strbuff)
	# rast$distProb = rep(0,length(mask))
	# rast$distProb[mask][cond] = exp(-0.1*(strbuff[cond]-1)*resolution)
	# writeRAST(rast, 'channelDistProb', zcol='distProb', overwrite=T)
