...
PROJDIR='/Volumes/LaCie/current_work/SEES/PondBranch' # full path to the project location;
...
RHESSysNAME='rhessys_pondbranch10m' # e.g., rhessys_baisman10m
...
downloadedDEMfile='BR_DEM1m_CBP.tif' # filename
...
gageLat='39.48031' # catchment outlet WSG84 Lat (decimal degree)
gageLong='-76.6875' # catchment outlet WSG84 Long (decimal degree; includes the negative sign if applied)
...
### ... river/stream threshold (by area) and expected drainage area
thres='180000' # meter squre 18 ha
expectedDrainageArea=349101 # meter squre (allow 2% error)
grass74 $LOCATION/$MAPSET --exec sh $PROJDIR/$RHESSysNAME/GIS2RHESSys-1mUrban/grass_delineation.sh $(bc <<< "$thres/$RESOLUTION/$RESOLUTION") $(bc <<< "0.98*$expectedDrainageArea/$RESOLUTION/$RESOLUTION") $(bc <<< "1.02*$expectedDrainageArea/$RESOLUTION/$RESOLUTION")
### ... set zone (optional)
grass74 $LOCATION/$MAPSET --exec Rscript $PROJDIR/$RHESSysNAME/GIS2RHESSys-1mUrban/zone_cluster.R dem slope aspect hill
### ... set riparian (optional)
grass74 $LOCATION/$MAPSET --exec r.buffer input=str output=riparian distances=10
###
...
downloadedLULCfile='BARN_1mLC_UTM.tif' # full path to the downloaded <file>
...
grass74 $LOCATIONLULC/$MAPSET --exec Rscript $PROJDIR/$RHESSysNAME/GIS2RHESSys-1mUrban/aggregate_lulcFrac.R patch$RESOLUTION'm' lulcRAW $PROJDIR/$RHESSysNAME/lulcFrac$RESOLUTION'm.csv'
grass74 $LOCATION/$MAPSET --exec Rscript $PROJDIR/$RHESSysNAME/GIS2RHESSys-1mUrban/aggregate_lulcFrac_write2GIS.R patch $PROJDIR/$RHESSysNAME/lulcFrac$RESOLUTION'm.csv'
###
...
grass74 $LOCATION/$MAPSET --exec Rscript $PROJDIR/$RHESSysNAME/GIS2RHESSys-1mUrban/g2w_subgridLULC.R $PROJDIR 101 clim/SLB.base $PROJDIR/$RHESSysNAME/GIS2RHESSys-1mUrban/vegCollection.csv $PROJDIR/$RHESSysNAME/GIS2RHESSys-1mUrban/soilCollection.csv $PROJDIR/$RHESSysNAME/GIS2RHESSys-1mUrban/lulcCollection.csv $PROJDIR/$RHESSysNAME/worldfiles/worldfile.csv $PROJDIR/$RHESSysNAME/worldfiles/worldfile.hdr $PROJDIR/$RHESSysNAME/defs
### ... flow table
grass74 $LOCATION/$MAPSET --exec Rscript $PROJDIR/$RHESSysNAME/GIS2RHESSys-1mUrban/createSubsurfaceFlowRouting.R $PROJDIR/$RHESSysNAME/flows/subsurfaceflow.txt
Rscript $PROJDIR/$RHESSysNAME/GIS2RHESSys-1mUrban/LIB_RHESSys_writeTable2World.R na $PROJDIR/$RHESSysNAME/worldfiles/worldfile.csv $PROJDIR/$RHESSysNAME/worldfiles/worldfile
