...
#----------------------------------------------------------------------------------------------
	...
	rast3 = readRAST('riparian')
		riparian = rast3@data[[1]][mask]	
	...
	subGridAssignment[2:4,1] = c(4.5,102,1) # tree [LAI, vegID, rootz]
	...
	patchVegnum = tapply(1:sum(mask),INDEX=tapplyOrder, FUN=function(x){
			...
			subGridAssignment[1,] = c(mean(forestFrac[x]),mean(shrubFrac[x]), mean(lawnFrac[x]), mean(impFrac[x]) )
				...
				if(is.na(subGridAssignment[1,4]) ) subGridAssignment[1,4] = 1 -subGridAssignment[1,1] -subGridAssignment[1,2] -subGridAssignment[1,3]
